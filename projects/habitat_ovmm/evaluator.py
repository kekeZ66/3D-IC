...
if TYPE_CHECKING:
    ...
    from home_robot.agent.ovmm_agent.mllm_agent import OpenVocabManipAgent
...
class OVMMEvaluator(PPOTrainer):
    ...
    def local_evaluate(
        ...
        while count_episodes < num_episodes:
            ...
            agent._sim = sim
            agent_init_position = (
                self._env.habitat_env.env.env.habitat_env.task._robot_start_position
            )
            agent_init_rotation = (
                self._env.habitat_env.env.env.habitat_env.task._robot_start_rotation
            )
            rotation_world_start = quaternion_from_coeff(
                agent_init_rotation
            )  # (x,y,z,w) -> (w,x,y,z)
            agent.rotation_world_start = rotation_world_start
            origin = np.array(agent_init_position)
            agent.habitat_origin = origin

            target_position = sim.target_start_pos[0]  # array
            agent.current_episode = current_episode_key

            current_episode_metrics = {}
            obs_data = [observations]

            while not done:
                agent.state = sim.get_agent_state()
                if agent.states[0] == 1:
