...
def create_ovmm_env_fn(config: "DictConfig") -> HabitatOpenVocabManipEnv:
    ...
    env_class_name = _get_env_name(config)      #  GymHabitatEnv
    env_class = get_env_class(env_class_name)   #  Args:env_name: name of the environment.    Returns:Type[habitat.RLEnv]: env class.
    habitat_env = env_class(config=habitat_config, dataset=dataset)
    habitat_env.seed(habitat_config.seed)
    env = HabitatOpenVocabManipEnv(habitat_env, config, dataset=dataset)   # habitat_env.step
    return env
