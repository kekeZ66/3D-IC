...
from home_robot.agent.objectnav_agent.feature_map_nav import ObjectNavAgent
...
class OpenVocabManipAgent(ObjectNavAgent):
    ...
    def __init__(self, config, device_id: int = 0):
        ...
        self.start_mllm_policy_pick = False
        self.start_mllm_policy_place = False
        self.point_list = []
        self.current_episode = None
        self.cache_goal_map = None
        self.target = None
        self.state = None
        self.rotation_world_start = None
        self.habitat_origin = None
        self.place_voxel = None
        ...
        skip_both_gaze = self.skip_skills.gaze_at_obj and self.skip_skills.gaze_at_rec  # heuristic: True
    ...
    def _get_info(self, obs: Observations) -> Dict[str, torch.Tensor]:
        ...
        use_detic_viz = self.config.ENVIRONMENT.use_detic_viz   # demo: True   val: False  
    ...
    def _place(
        ...
        if self.skip_skills.place:
            ...
            action, info = self.place_policy(obs, info, self.place_info)
        elif place_type == "rl":
    ...
    def act(
        self, obs: Observations, sim = None
        ...
        if self.config.GROUND_TRUTH_SEMANTICS == 0:
            ...
            # camera_K camera_pose # depth (640, 480) rgb (640,480,3) joint (10)  # task_observations {'object_embedding':512 } # semantic (640, 480)
        else:
