...
from home_robot.navigation_policy.object_navigation.mllm_policy import ObjectNavFrontierExplorationPolicy

# Do we need to visualize the frontier as we explore?
debug_frontier_map = False


class ObjectNavAgentModule(nn.Module):   # semantic map + frontier explore
    def __init__(self, config, instance_memory: Optional[InstanceMemory] = None):
        ...
        self.waypoint_sample_region = None
    ...
    def forward(
        ...
        self.map_features = seq_map_features.flatten(0, 1)
        ...
        goal_map, found_goal = self.policy(
            self.map_features,
            self.semantic_map_module.frontiers,
            self.semantic_map_module._value_map_local,
            self.semantic_map_module._value_map_recp_local,
            self.semantic_map_module.local_frontier_map,
            self.semantic_map_module.local_vlfm_explore_map,
        ...
        self.frontiers = self.semantic_map_module.frontiers

        origin = seq_origins[0,0,:2].cpu().numpy()  
        map_frontier_pt = self.policy.map_frontier_pt
        self.relative_frontiers_m = [(x / 20.0 - origin[0], y / 20.0 - origin[1]) for (x,y) in map_frontier_pt]

        self.value_map_obj = self.semantic_map_module._value_map_local
        self.value_map_recep = self.semantic_map_module._value_map_recp_local
        self.detect_frontier_map = self.semantic_map_module.local_frontier_map
        self.vlfm_explored_map = self.semantic_map_module.local_vlfm_explore_map
        ...
        if found_goal and self.policy.goal_reach_mask is not None:
            goal_reach_mask = self.policy.goal_reach_mask

        # Compute the frontier map here
        frontier_map = self.policy.get_frontier_map(self.map_features)
