...
class DiscretePlanner:
    ...
    def __init__(
        ...
        self.change_goal_to_frontier = False
        self.goal_reach_mask = None
        self.gt_goal_map = None
        self.gt_recep_map = None

        self.first_found_goal = False
        self.data_collect_pick_step = 0
    ...
    def plan(
        ...
        if self.goal_reach_mask is not None:
            self.goal_reach_mask = self.goal_reach_mask.squeeze(0).cpu().numpy()
            self.goal_reach_mask = add_boundary(self.goal_reach_mask)
        ...
        if self.visited_count_map[gx1:gx2, gy1:gy2][start[0],start[1]] > 15:
            self.stuck = True
            if (self.visited_count_map[gx1:gx2, gy1:gy2][start[0]-5: start[0]+6,start[1]-5: start[1]+6]).sum() > 80:
                self.change_goal_to_frontier = True
        else:
        ...
        if self.last_action == DiscreteNavigationAction.MOVE_FORWARD or (
            ...
            self._check_collision(found_goal)
        ...
        return action, closest_goal_map, short_term_goal, dilated_obstacles, self.change_goal_to_frontier
    ...
    def _get_short_term_goal(
        ...
        if plan_to_dilated_goal: 
            ...
            goal_distance_map, closest_goal_pt = self.get_closest_traversible_goal( # new FMMplanner
            ...
            if found_goal:
                ...
                navigable_goal_map = planner._find_within_distance_to_multi_goal( 
                    ...
                    goal_reach_mask = self.goal_reach_mask
                )
                # wall
                if (traversible[goal_map == 1]).sum() == 0:
                    height_map = add_boundary(height_map,value=0)
                    wall = (height_map > 0.7).astype(np.uint8)
                    wall = cv2.dilate(wall, self.obs_dilation_selem,iterations=1)
                    navigable_goal_map_in_same_room = los_exists_visible_mask_numpy(wall, goal_map, navigable_goal_map)
                    if navigable_goal_map_in_same_room.sum() > 0:
                        navigable_goal_map = navigable_goal_map & navigable_goal_map_in_same_room
            else:
                navigable_goal_map = planner._find_within_distance_to_multi_goal(
                    goal_map,
                    self.min_goal_distance_cm / self.map_resolution,
        ...
        self.step_fmm_dist = remove_boundary(self.dd)[start[0],start[1]]
        self.timestep += 1
    ...
    def _check_collision(self,found_goal):
        ...
        if abs(x1 - x2) < 0.05 and abs(y1 - y2) < 0.05:
            ...
            if self.stuck_time > 20 and not found_goal:
                self.col_width = 10
                length = 10
                buf = 5
        else:
    ...
    def get_gt_distance(self, traversible):
        gt_planner = FMMPlanner(
            np.ones_like(traversible),
            print_images=False,
            vis_dir=None,
        )
        gt_fmm_dist = gt_planner.set_multi_goal(add_boundary(self.gt_goal_map, value=0))
        gt_fmm_dist = remove_boundary(gt_fmm_dist)
        return gt_fmm_dist
