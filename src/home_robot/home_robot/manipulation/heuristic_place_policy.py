...
class HeuristicPlacePolicy(nn.Module):
    ...
    def __init__(
        ...
        self.keypoint = None
        self.continuous_adjustment = 0.0
        self.last_gps = None
        self.move_fwd_total = 0
        # self.mllm = OllamaMLLM()
    ...
    def get_receptacle_placement_point(
        ...
        if not goal_rec_mask.any():
            ...
            if place_info is not None:
                idx_map = place_info['idx_map']
                height_map = place_info['height_map']
                for point in random_indices:
                    idx_in_idxmap = idx_map[point[0],point[1]]
                    point_height = idx_in_idxmap[2].cpu()/1000
                    grid_height = height_map[idx_in_idxmap[1],idx_in_idxmap[0]] * 1.5
                    if abs(point_height - grid_height) < 0.1:
                        surface_point.append(point)  
                if len(surface_point) == 0:
                    surface_point = random_indices
                ground_map = (height_map < 0.05)

                from home_robot.utils.nav_utils import nearest_one_point_edt
                max_dist = 0
                for point in surface_point:
                    idx_in_idxmap = idx_map[point[0],point[1]].cpu().numpy()
                    r, c = idx_in_idxmap[1],idx_in_idxmap[0]
                    
                    npt = nearest_one_point_edt(ground_map, (r,c))
                    dist = (npt[0] - r) ** 2 + (npt[1] - c) ** 2
                    if dist > max_dist:
                        max_point = point    
            ...
            for ind in random_indices:
                ...
                surface_point_list.append(slab_points_mask.sum().cpu().numpy())
            ...
            if max_point is not None:
                best_voxel_ind = max_point
                best_voxel = pcd_base_coords[best_voxel_ind[0],best_voxel_ind[1]]  
            self.keypoint = best_voxel_ind
    ...
    def forward(self, obs: Observations, vis_inputs: Optional[Dict] = None, place_info = None):
        """
        ...
        if self.timestep == 0:
            ...
            found = self.get_receptacle_placement_point(obs, vis_inputs, place_info)
            ...
            if found is None:
                ...
                self.turn_angle_rad = get_angle_to_pos(center_voxel_trans)

                self.initial_orient_num_turns = abs(delta_heading) // turn_angle
                self.initial_orient_num_turns = 1.0

                self.orient_turn_direction = np.sign(delta_heading)
                import math
                fwd = math.sqrt(self.placement_voxel[0]**2 + self.placement_voxel[1]**2)
                fwd_dist = (
                    self.placement_voxel[1]
                ...
                fwd_dist = np.clip(fwd_dist, 0, np.inf)  
                fwd_step_size = 0.1
                ...
                if abs(self.placement_voxel[0]) > 0.25:
                    if self.last_gps is None or np.linalg.norm(obs.gps - self.last_gps) > 0:
                        self.continuous_adjustment = 3.0
                        self.total_turn_and_forward_steps  = self.forward_steps + 3.0
        ...
        if self.timestep < self.initial_orient_num_turns:
            ...
            action = ContinuousNavigationAction(np.array([0.1,0,0]))
        elif self.timestep == self.total_turn_and_forward_steps:
            ...
            import math
            placement_extension = math.sqrt(self.placement_voxel[0]**2 + self.placement_voxel[1]**2)
        ...
        if self.continuous_adjustment > 0:
            if self.timestep == 0:
                action = ContinuousNavigationAction(np.array([0,0,self.orient_turn_direction * 1.57]))
            elif self.timestep == 1:
                action = ContinuousNavigationAction(np.array([abs(self.placement_voxel[0]),0,0]))
            elif self.timestep == 2:
                action = ContinuousNavigationAction(np.array([0,0,self.orient_turn_direction * (- 1.57)]))
                self.timestep = -1
                self.continuous_adjustment = 0
                self.move_fwd_total = 0
