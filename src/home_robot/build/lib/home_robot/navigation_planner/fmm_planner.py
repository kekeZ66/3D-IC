...
class FMMPlanner:
    ...
    def set_multi_goal(
        ...
        if (self.visualize or self.print_images) and timestep != 0:
            ...
            trav_u = np.flipud(traversible.astype(np.uint8))            # 0/1
            goal_u = np.flipud((goal_map > 0).astype(np.uint8))         # 0/1

            dd = self.fmm_dist
            denom = float(dd.max()) if dd is not None and dd.max() > 0 else 1.0
            dd_u = np.flipud((dd / denom * 255.0).astype(np.uint8))     # 0..255

            # 单通道转 BGR 以便做彩色叠加
            left  = cv2.cvtColor((trav_u * 255), cv2.COLOR_GRAY2BGR)    # 可通行为白、障碍为黑
            right = cv2.cvtColor(dd_u,            cv2.COLOR_GRAY2BGR)   # 距离场灰度
            center = cv2.cvtColor((goal_u * 255), cv2.COLOR_GRAY2BGR)   # 纯 goal 二值图

            # 在左右两侧叠加 goal（黄色半透明）
            alpha = 0.6
            yellow = np.zeros_like(left, dtype=np.uint8)
            yellow[goal_u == 1] = (0, 255, 255)  # BGR: Yellow

            left  = cv2.addWeighted(yellow, alpha, left,  1 - alpha, 0)
            right = cv2.addWeighted(yellow, alpha, right, 1 - alpha, 0)

            # 横向拼接：左=traversible+goal， 中=goal， 右=dist+goal
            vis = np.concatenate([left, center, right], axis=1)

            if self.visualize:
                cv2.imshow("Planner Distance (goal overlay L/R)", vis)
                cv2.waitKey(1)

            if self.print_images and timestep is not None:
                os.makedirs(self.vis_dir, exist_ok=True)
                # out_path = os.path.join(self.vis_dir, f"planner_snapshot_{timestep}.png")
                out_path = os.path.join(self.vis_dir, f"planner_snapshot_{timestep}.png")
    ...
    def _find_within_distance_to_multi_goal(
        ...
        if goal_obstacle.sum() > 0:
            if not min_distance_only:
                ...
                distance = 8.0
                navigable_obastacle_goal_map = dist_map < distance
                if navigable_obastacle_goal_map.sum() < 10:
                    distance = 10.0
                    navigable_obastacle_goal_map = dist_map < distance
        else:
