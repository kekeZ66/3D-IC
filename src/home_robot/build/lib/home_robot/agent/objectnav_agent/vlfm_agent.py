...
from home_robot.vlfm.vlfm.policy.habitat_policies import HabitatITMPolicyV2
import os
from home_robot.vlfm.vlfm.vlm.blip2itm import BLIP2ITMClient
...
class ObjectNavAgent(Agent):
    ...
    def __init__(
        ...
        #     min_depth = 0.5,
        #     max_depth = 3.5,
        #     camera_fov = config.ENVIRONMENT.hfov,
        #     image_width = 480,
        #     text_prompt = "Seems like there is a target_object ahead.",
        #     use_max_confidence = False,
        #     sync_explored_areas = False,
        #     pointnav_policy_path = "/home/zkm/projects/vlfm/data/pointnav_weights.pth",
        ...
        #     agent_radius = config.AGENT.radius,
        #     exploration_thresh = 0.0
        # )

        # self.prev_action = torch.tensor([[0]], dtype=torch.long)
        ...
        Returns:
            ...
            vis_inputs: list of num_environments visualization info dicts containing
                ...
                semantic_map: (M, M) np.ndarray containing local semantic map
                 predictions
        """
        dones = torch.tensor([False] * self.num_environments) # 多个env 不同阶段？
    ...
    # def preprocess_vlfm(self, obs):
    #     keys_to_keep = ["base_explorer","compass", "depth", "frontier_sensor" ,"gps", "heading", "objectgoal", "rgb"]
    #     tensor_dict = {}
    #     for key in keys_to_keep:
    #         if key in obs:  # obs里可能有些字段不存在
    #             val = torch.from_numpy(obs[key]).unsqueeze(0).to(self.device)
    #             tensor_dict[key] = torch.as_tensor(val, dtype=torch.float32)
    ...
    def act(self, obs: Observations) -> Tuple[DiscreteNavigationAction, Dict[str, Any]]:
        ...
        obstacle_locations = None      # None
        free_locations = None      # None

        # vlfm_input = self.preprocess_vlfm(obs, camera_pose)

        # if self.timesteps[0] == 1:
        #     vlm_in_mask = torch.tensor([[False]], dtype=torch.bool).to(self.device)
        # else:
        #     vlm_in_mask = torch.tensor([[True]], dtype=torch.bool).to(self.device)
        
        # VLFMPolicyActionData = self.vlfm.act(
        #     observations=vlfm_input,
        ...
        #     deterministic = False
        # )
        # vlfm_action = VLFMPolicyActionData.actions
        # vlfm_action = self.vlfm_action_to_ovmm(vlfm_action)
        # print("vlfm_action:", vlfm_action)

        # ----------------compute vlfm value----------------------
        vlfm_input_rgb = np.array(obs.rgb)
        text_goals = goal_name[0].split()
        obj_name = text_goals[1]
        start_recp = text_goals[3]
        end_recp = text_goals[5]
        find_object_phase_prompt = f"{obj_name} on {start_recp}"
        vlfm_input_text_prompt = self.text_prompt.replace("target_object", find_object_phase_prompt)
        cosine1 = self._itm.cosine(vlfm_input_rgb, vlfm_input_text_prompt)
        place_object_phase_prompt = self.text_prompt.replace("target_object",end_recp)
        cosine2 = self._itm.cosine(vlfm_input_rgb, place_object_phase_prompt)
        ...
        if "semantic_max_val" in obs.task_observations:
            semantic_max_val = obs.task_observations["semantic_max_val"] # 语义类别编号的最大值 4
    ...
    def vlfm_action_to_ovmm(self, vlfm_action):
        if vlfm_action[0][0] == 0:
            ovmm_action = DiscreteNavigationAction.STOP
        elif vlfm_action[0][0] == 1:
            ovmm_action = DiscreteNavigationAction.MOVE_FORWARD
        elif vlfm_action[0][0] == 2:
            ovmm_action = DiscreteNavigationAction.TURN_LEFT
        else:
            ovmm_action = DiscreteNavigationAction.TURN_RIGHT
        return ovmm_action